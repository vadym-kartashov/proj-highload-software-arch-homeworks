...
function setup_mysql_slave() {
  local slave_container="$1"

  until docker-compose exec "$slave_container" sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
  do
      echo "Waiting for ${slave_container} database connection..."
      sleep 4
  done
  ...
  docker exec "$slave_container" sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"
}
...
setup_mysql_slave mysql_slave1
setup_mysql_slave mysql_slave2

# Example usage:
# setup_mysql_slave mysql_slave
